...
@interface STInputTextViewController () <UITextViewDelegate,UITextFieldDelegate>
{
...
@implementation STInputTextViewController
...
- (void)viewDidLoad {
    ...
    if (_limitedTextLength > 0 && !_textLimitLabel) {
        _textLimitLabel = [[UILabel alloc] init];
        _textLimitLabel.text = [NSString stringWithFormat:@"还可以输入%ld个字", _limitedTextLength - self.text.length];
        _textLimitLabel.font = [UIFont systemFontOfSize:14.];
        _textLimitLabel.hidden = _limitedTextLength == 0;
